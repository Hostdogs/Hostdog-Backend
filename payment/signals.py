...
def create_payment_post_save(sender, created, instance, update_fields, **kwargs):
    ...
    if instance.main_status == "payment" and not instance.created_deposit_payment:
        ...
        service_delta = (
            localtime(instance.service_end_time).date()
            - localtime(instance.service_start_time).date()
        )
        ...
        for date in all_date_within_interval:
            for feedingtime in dog_feeding_time_object:
                ...
                if (
                    localtime(instance.service_start_time)
                    < make_aware(date_feeding)
                    < localtime(instance.service_end_time)
                ):
                    meal_per_service += 1
        ...
        print(
            "host_service_instance.deposit_price", host_service_instance.deposit_price
        )
